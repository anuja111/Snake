...
maxx,maxy,speed=100,bo=10,t[10],score=0,hscore=20;
   ...
   int k=2,l,f=0,z=100,first=0,second=1;
  int r1=300;
   int r2=350;

    char *main_menu[]={{"1. Play"},{"2. Instruction"},{"3. Exit"}};
    char *m_m[]={"P","I","E"};
 void intro_graphics();
void show_main_menu();
void get_mm_choice();
...
void intro_graphics()
{
  ...
  settextstyle(1,0,4);
  setcolor(GREEN);
  outtextxy(130,200,"Press any key to contiue");
  getch();
}
void showtime()
{
...
_strtime(timebuf);
setcolor(CYAN);
outtextxy(maxx-140,420,"date");
outtextxy(maxx-80,420,datebuf);
outtextxy(maxx-140,440,"time");
outtextxy(maxx-70,440,timebuf);
}
void show_main_menu()
{
 cleardevice();
...
void instruction()
{
 ...
 setcolor(RED);
 settextstyle(1,0,5);
 outtextxy(50,140,"INSTRUCTIONS");
 settextstyle(3,0,3);
 setcolor(WHITE);
 outtextxy(60,210,"1. Move the snake using number keys 8 ,5,4,6.");
  outtextxy(60,270,"2. Eat food  and make snake grow longer.");
  outtextxy(60,330,"3.Don't let it hit the wall.");
  getch();
 show_main_menu();
}
 void initialise()
...
settextstyle(10,0,6);
 ...
 move:
   ...
   bar(4,4,maxx-154,10);
   bar(4,4,10,maxy-4);
   bar(4,maxy-4,maxx-154,maxy-10);
   bar(maxx-154-5,4,maxx-154,maxy-4);
   }
   ...
   setcolor(15);
   outtextxy(maxx-130,230,"Player's Name");
   setcolor(15);
   outtextxy(maxx-100,250,"ARA");
   setcolor(1);
   delay(200);
   itoa(score,scor,10);
   outtextxy(maxx-140,280,"Score");
   outtextxy(maxx-50,280,scor);
   delay(200);
   setcolor(10);
   setcolor(14);
   outtextxy(maxx-140,320,"High Score  ");
   outtextxy(maxx-50,320,hs);

   for(j=0; j<=1000; j++)
   {
	putpixel(random(maxx-150),random(maxy),1);
	delay(1);
   }
    while (a!='0')
   {
      ...
      {
	setcolor(13);
	outtextxy(maxx-140,380,"Press Any Key...");
	getch();
	showtime();
	setcolor(13);
	outtextxy(maxx-140,380,"Press Any Key...");
		...
		if(x<=14 || x>=maxx-184 || y<=14 || y>=maxy-14)
		  ...
		  else
		  {
			  ...
			  setcolor(15);
			   ...
			   setcolor(11);
			   outtextxy(maxx-140,420,"YOUR FINAL SCORE IS");
			   outtextxy(maxx-100,430,scor);
			   sound(100);
			   delay(400);
			   nosound();
			   getch();
